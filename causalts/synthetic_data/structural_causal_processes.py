...
def _generate_noise(covar_matrix, time=1000, use_inverse=False):
    ...
    return np.random.multivariate_normal(
        mean=np.zeros(n_nodes), cov=this_covar, size=time
    )
...
def _check_stability(graph):
    ...
    stability_matrix = scipy.sparse.hstack(
        [scipy.sparse.lil_matrix(graph[:, :, t_slice]) for t_slice in range(period)]
    )
    ...
    if n_eigs <= 25:
        ...
        eigen_values = scipy.sparse.linalg.eigs(
            stability_matrix, k=(n_eigs - 2), return_eigenvectors=False
        )
    # Ensure they all have less than one magnitude
    assert np.all(np.abs(eigen_values) < 1.0), (
...
def _var_network(
    graph, add_noise=True, inno_cov=None, invert_inno=False, T=100, initial_values=None
):
    """
    ...
    if add_noise:
        ...
        if inno_cov is not None:
            noise = _generate_noise(inno_cov, time=time, use_inverse=invert_inno)
...
def _check_symmetric_relations(a_matrix):
    ...
    if not np.allclose(a_matrix, a_matrix.T, rtol=1e-10, atol=1e-10):
        ...
        raise ValueError(
            "Relationships between nodes at tau=0 are not"
            + " symmetric!\n"
            + error_message
        )
...
def _get_covariance_matrix(parents_neighbors_coeffs):
    ...
    _, max_node_id = _find_max_time_lag_and_node_id(parents_neighbors_coeffs)
...
def _get_lag_connect_matrix(parents_neighbors_coeffs):
    ...
    max_time_lag, max_node_id = _find_max_time_lag_and_node_id(parents_neighbors_coeffs)
    n_nodes = max_node_id + 1
...
def structural_causal_process(
    ...
    if func_map is None:
        raise ValueError("function map is needed")
    ...
    if intervention is not None:
        ...
        for j in intervention.keys():
            if len(intervention[j]) != T:
                raise ValueError(
                    "intervention array for j=%s must be of length T = %d" % (j, T)
                )
            if j not in intervention_type.keys():
                raise ValueError(
                    "intervention_type dictionary must contain entry for %s" % (j)
                )
    ...
    for t in range(max_lag, T + transient):
        for j in causal_order:
            if (
                ...
                and j in intervention
                and t >= transient
                and not np.isnan(intervention[j][t - transient])
            ):
    ...
    for j in range(N):
        for link_props in links[j]:
            if len(link_props) > 2:
                ...
                coeff = link_props[1]
                # func = link_props[2]
                if not isinstance(coeff, float) or coeff != 0.0:
...
def generate_structural_causal_process(
    ...
    # def linear_func(x): return x
    # def nonlinear(x): return (x + 5. * x**2 * np.exp(-x**2 / 20.))

    if max_lag == 0:
        contemp_fraction = 1.0
    ...
    contemp_links = [
        (causal_order[pair[0]], causal_order[pair[1]])
    ...
    class NoiseModel:
        ...
        def weibull(self, T):
            ...
            variance = (
                scipy.special.gamma(2.0 / a + 1) - scipy.special.gamma(1.0 / a + 1) ** 2
            )
            return self.mean + self.sigma * (
                random_state_noise.weibull(a=a, size=T) - mean
            ) / np.sqrt(variance)
        ...
        def uniform(self, T):
            ...
            variance = 1.0 / 12.0
            return self.mean + self.sigma * (
                random_state_noise.uniform(size=T) - mean
            ) / np.sqrt(variance)
