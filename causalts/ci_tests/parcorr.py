...
class ParCorr(CondIndTest):
    ...
    **kwargs :
    ...
    def __init__(self, **kwargs):
        ...
        self.n_tests = 0
        CondIndTest.__init__(self, method="par_corr", **kwargs)
        self._measure = "par_corr"
        self.method = "par_corr"
    ...
    def get_analytic_confidence(self, value, df, conf_lev):
        ...
        conf_lower = stats.t.ppf(q=1.0 - c_int, df=df, loc=value_tdist) / np.sqrt(
    ...
    def get_model_selection_criterion(self, j, parents, tau_max=0, criterion="aic"):
        ...
        if criterion == "corrected_aic":
            score = T * np.log(rss) + 2.0 * p + (2.0 * p**2 + 2.0 * p) / (T - p - 1)
        elif criterion == "bic":
            score = T * np.log(rss / float(T)) + p * np.log(
                T
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if param_hash is None:
            T = data.shape[0] if data is not None else 0
            param_hash = f"T{T}-analytic"
        super().__init__(
            seed=42,
